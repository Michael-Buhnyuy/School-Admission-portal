...
static::$instances['autoloader'] = new Autoloader(new \CodeIgniter\Config\AutoloadConfig(), new \CodeIgniter\Modules\Modules());
            }

            return static::$instances['autoloader'];
        }

return new Autoloader(new \CodeIgniter\Config\AutoloadConfig(), new \CodeIgniter\Modules\Modules());
    }
    ...
    protected static function buildServicesCache(): void
    {
        if (! static::$discovered) {
            if ((new Modules())->shouldDiscover('services')) {
                ...
                $systemPaths = static::autoloader()->getNamespace('CodeIgniter');
                $systemPath  = $systemPaths[0] ?? null;

                if ($systemPath === null) {
                  return; // or throw a clearer exception about missing CodeIgniter namespace
}
