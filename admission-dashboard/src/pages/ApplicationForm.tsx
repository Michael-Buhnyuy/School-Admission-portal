...
const ApplicationForm = () => {
  ...
  const validateStep = (step: number) => {
    ...
    if (step === 2) {
      if (!formData.primarySchool)
        newErrors.primarySchool = "Primary school is required";
      if (!formData.primaryYear) newErrors.primaryYear = "Year is required";
      if (!formData.secondarySchool)
        newErrors.secondarySchool = "Secondary school is required";
      if (!formData.secondaryYear) newErrors.secondaryYear = "Year is required";
    }

    if (step === 3) {
      if (!formData.selectedProgram)
        newErrors.selectedProgram = "Program is required";
  ...
  const sendOtp = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8080/api/applicant/send-otp",
        {
          method: "POST",
      ...
      if (data.success) {
        ...
        setSubmitMessage(data.message || "OTP send failed");
      }
    } catch (error) {
      setSubmitMessage("Network error. Backend may not be running.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const verifyOtpAndSubmit = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8080/api/applicant/register",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (data.success) {
        setSubmitMessage(
          "Application submitted successfully! Check your email for OTP.",
        );
        setOtpSent(true);
      } else {
      ...
      setSubmitMessage("Network error. Please check backend.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    const selectedProgramObj = programs.find(
      (p: any) => p.id === Number(formData.selectedProgram),
    );
    const selectedProgramName = selectedProgramObj?.name || "Unknown Program";
            ...
            <div className="form-grid">
              ...
              <div className="input-group full-width">
                ...
                {errors.secondarySchool && (
                  <span className="error-text">{errors.secondarySchool}</span>
                )}
              </div>
              ...
              <div className="input-group">
                ...
                {errors.secondaryYear && (
                  <span className="error-text">{errors.secondaryYear}</span>
                )}
              </div>
            ...
            <div className="form-grid">
              <div className="input-group full-width">
                ...
                {errors.selectedProgram && (
                  <span className="error-text">{errors.selectedProgram}</span>
                )}
              </div>
              ...
              <div className="input-group">
                ...
                {errors.campus && (
                  <span className="error-text">{errors.campus}</span>
                )}
              </div>
              ...
              <div className="input-group">
                ...
                {errors.intake && (
                  <span className="error-text">{errors.intake}</span>
                )}
              </div>
            </div>
          </motion.div>
            ...
            <div className="review-sections">
              <div className="review-section">
                ...
                <div className="review-grid">
                  ...
                  <div>
                    <span>Address:</span> {formData.address}, {formData.city},{" "}
                    {formData.state}
                  </div>
                </div>
              </div>
              ...
              <div className="review-section">
                <div className="review-header">
                  <h3>
                    <GraduationCap size={18} /> Educational Details
                  </h3>
                  <button
                    className="edit-btn"
                ...
                <div className="review-grid">
                  <div>
                    ...
                    {formData.primarySchool || "N/A"}
                  </div>
                  <div>
                    <span>Secondary School:</span> {formData.secondarySchool}
                  </div>
                  <div>
                    <span>Graduation Year:</span> {formData.secondaryYear}
                  </div>
                  <div>
                    <span>Tertiary:</span>{" "}
                    {formData.tertiaryInstitution || "N/A"}
                  </div>
                </div>
              </div>
              ...
              <div className="review-section">
                <div className="review-header">
                  <h3>
                    <BookOpen size={18} /> Program Selection
                  </h3>
                  <button
                    className="edit-btn"
                ...
                <div className="review-grid">
                  <div>
                    <span>Program:</span>{" "}
                    {programs.find((p) => p.id === formData.selectedProgram)
                      ?.name || "N/A"}
                  </div>
                  <div>
                    <span>Campus:</span> {formData.campus || "N/A"}
                  </div>
                  <div>
                    <span>Intake:</span> {formData.intake || "N/A"}
                  </div>
                </div>
              </div>
            </div>
